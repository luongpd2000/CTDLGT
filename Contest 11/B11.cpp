...
bool isPerfectTree(Node *root){
    ...
    while(!q.empty()){
        ...
        if(f->pLeft==nullptr && f->pRight==nullptr && level==-1){
            level=f->level;
        }
        else if(f->pLeft==nullptr && f->pRight==nullptr && f->level != level){
            return false;
        }
        else if((f->pLeft==nullptr && f->pRight!=nullptr) ||
...
main(){
    ...
    while(t--){
        ...
        cout<<(isPerfectTree(root)?"Yes":"No")<<endl;
        delTree(root);
    }
}
